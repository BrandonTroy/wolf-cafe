...
public class UserController {
	...
	private ResponseEntity<UserDto> validate(String action, Long id, UserDto userDto) throws NoSuchAlgorithmException {
		...
		if (userDto.getUsername().trim().isEmpty() || userDto.getEmail().trim().isEmpty()
				|| userDto.getPassword().trim().isEmpty() || userDto.getPassword().length() < 8) {
